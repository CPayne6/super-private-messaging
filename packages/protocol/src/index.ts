...
/** A server-issued, one-use challenge. The client signs `signingBytes(request)`.
 * `challengeId` is represented by `requestId` in the signed input so it cannot be
 * swapped by an HTTP intermediary. */
...
export function normalizeUsername(input: string): string {
  ...
  if (!USERNAME.test(username))
    throw new ProtocolError(
      ...
      "Username must be 3–32 lowercase handle characters.",
    );
  return username;
}
...
/** Reject malformed public material at every adapter boundary. */
export function validatePrekeyBundle(
  bundle: PublicPrekeyBundle,
): PublicPrekeyBundle {
  normalizeUsername(bundle.username);
  if (
    ...
    throw new TypeError(
      "Prekey identifiers and key version must be non-negative safe integers.",
    );
  assertByteLength(
  ...
  if (
    ...
    throw new TypeError("One-time prekey and ID must be supplied together.");
  if (bundle.oneTimePrekey) {
    ...
    const oneTimePrekeyId = bundle.oneTimePrekeyId;
    if (
      typeof oneTimePrekeyId !== "number" ||
      !Number.isSafeInteger(oneTimePrekeyId) ||
      oneTimePrekeyId < 0
    )
      throw new TypeError(
        "One-time prekey ID must be a non-negative safe integer.",
      );
  }
  return bundle;
}
...
export function validateSignedRequest(
  ...
  if (
    ...
    !["private-http", "socket-connect", "socket-send"].includes(request.purpose)
  )
    throw new TypeError("Unsupported signed request.");
  if (
    !request.method ||
    ...
    throw new TypeError("Signed request is missing a required binding.");
  assertByteLength(request.payloadHash, SHA256_BYTES, "Payload hash");
...
export function validateEnvelope(envelope: DirectEnvelope): DirectEnvelope {
  if (
    envelope.version !== PROTOCOL_VERSION ||
    !envelope.idempotencyId ||
    typeof envelope.prekeyMessage !== "boolean" ||
  ...
  normalizeUsername(envelope.recipient);
  if (
    !Number.isSafeInteger(envelope.recipientKeyVersion) ||
    envelope.recipientKeyVersion < 1 ||
    envelope.ciphertext.byteLength === 0 ||
    envelope.ciphertext.byteLength > MAX_MESSAGE_BYTES
  )
    throw new ProtocolError(
      ErrorCode.ENVELOPE_INVALID,
  ...
  const write = (v: unknown): void => {
    ...
    if (typeof v === "object" && v) {
      ...
      const entries = Object.entries(v as Record<string, unknown>)
        .map(([k, x]) => [enc.encode(k), k, x] as const)
        .sort((a, b) => compareBytes(a[0], b[0]));
      pushHead(5, entries.length);
...
export function signingBytes(request: SignedRequest): Bytes {
  return canonicalCbor(request);
}
export function envelopeBytes(envelope: DirectEnvelope): Bytes {
  return canonicalCbor(envelope);
}
/** Bytes signed during the native WebSocket challenge-response handshake. */
export function websocketChallengeBytes(
  username: string,
  nonce: Bytes,
  expiresAt: number,
): Bytes {
...
export { protocolFixtures } from "./fixtures.js";
